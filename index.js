...
const main = async () => {
    ...
    const b = new Date('2022-07-03T21:58:08.315Z').toISOString()
    const d = new Date('2022-07-04T21:58:08.315Z').toISOString()
    for (let index = 0; index < merchants.length; index++) {
        const merchant = merchants[index];
        const [rev, revStatus] = await client.getMerchantReviews({ merchantId: merchant.id, dateFrom: b, dateTo: d, pageSize: 50 })
        ...
        // const [order, orderStatus] = await client.getOrderById('2aaea4d0-7a13-4b22-b976-6cd13edd5ea6')
